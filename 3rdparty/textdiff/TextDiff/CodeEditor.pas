...
uses
  Windows, Messages, types, SysUtils, ShellApi, Classes, Forms,
...
type
  ...
  TPaintLineEvent = procedure (Sender: TObject; LineNo: integer;
    Rec: TRect; TextLeft: integer; var Handled: boolean) of object;
  ...
  protected
    ...
    property UndoList: TUndo read fUndo;
  public
    ...
    property Partner: TCodeEdit read fPartner write fPartner;
  published
    ...
    property OnPaintLine: TPaintLineEvent read fOnPaintLine write fOnPaintLine;
...
begin
  ...
  try
    ...
    for i := 0 to Strings.Count - 1 do Insert(i,Strings[i]);
  finally
    EndUpdate;
  end;
end;
      ...
      case UndoType of
        ...
        utSetSel:
          begin
            ...
            if (fUndoPos > 0) and
              (PUndoItem(fUndoList[fUndoPos-1]).UndoType = utDelSel) then DoUndo;
          end;
        utAddChar:
          begin
            //undo Add Char ...
        ...
        utDelChar:
          begin
            ...
            fSelEnd := OldSelEnd;
            fCaretPt := OldCaretPt;
            KeyPressWhileEditing(DeletedText[1]);
            if AddedText[1] = DEL then //as opposed to BACKSPC
            begin
              CaretPt := OldCaretPt;
              fSelStart := OldSelEnd;
            end;
      ...
      case UndoType of
        utDelSel:
          begin
            ...
            if (fUndoPos < fUndoList.Count-1) and
              (PUndoItem(fUndoList[fUndoPos+1]).UndoType = utSetSel) then
            begin
              inc(fUndoPos);
              DoRedo;
              exit;
        ...
        utAddChar:
          begin
            ...
            fSelEnd := OldSelEnd;
            fCaretPt := OldCaretPt;
            KeyPressWhileEditing(AddedText[1]);
          end;
        utDelChar:
          begin
            //redo Delete Char ...
  ...
  begin
    ...
    begin
      ...
      while (tmpCharPt.X < fLines.LineObj[tmpCharPt.Y].fLineLen) and
        CharInSet(fLines[tmpCharPt.Y][tmpCharPt.X+1], ['0'..'9','A'..'Z','a'..'z']) do
          inc(tmpCharPt.X);
...
{$IFNDEF VER100}
function TCodeEdit.DoMouseWheel(Shift: TShiftState; WheelDelta: Integer;
      MousePos: TPoint): Boolean;
begin
  //mousewheel scrolls a 1/4 of page ...
  if WheelDelta < 0 then
    InternalVScroll(fVertOffset + VisibleLines div 4) else
    InternalVScroll(fVertOffset - VisibleLines div 4);
  Result:= true;
end;
{$ENDIF}
//------------------------------------------------------------------------------
...
begin
  ...
  case Key of
    ...
    VK_ESCAPE: if fMouseAction = maSelMove then
               begin
                 MouseCapture := false;
                 fMouseAction := maNone;
                 cursor := crIBeam;
  ...
  if not (Key in [VK_SHIFT .. VK_CAPITAL]) then ScrollCaretIntoView;
...
var
  ...
  UndoItem: TUndoItem;
begin
  if (SelLength = 0) or ReadOnly then exit;
  selStartPt := CharPtFromCharPos(SelStart);
  selEndPt := CharPtFromCharPos(SelStart + SelLength);

  UndoItem.UndoType := utDelSel;
  UndoItem.OldSelStart := SelStart;
  UndoItem.OldSelEnd := SelStart + SelLength;
  UndoItem.DeletedText := Selection;

  if (selStartPt.X = 0) and (selStartPt.Y = 0) and
    (selEndPt.Y = Lines.Count-1) and
    (selEndPt.X = Lines.LineObj[selEndPt.Y].LineLen) then
  begin
    //everything selected so just clear and exit ...
    Lines.Clear;
    UndoItem.NewSelStart := 0;
    UndoItem.NewCaretPt := Point(0,0);
  ...
  Lines.BeginUpdate;
  try
    sLead := copy(Lines[selStartPt.Y],1,selStartPt.X);
    sTrail := copy(Lines[selEndPt.Y],selEndPt.X+1,$FFFF);
    if selStartPt.X = 0 then
    begin
      for i := selEndPt.Y-1 downto selStartPt.Y do Lines.Delete(i);
      if selEndPt.X > 0 then Lines[selStartPt.Y] := sTrail;
    end else
    begin
      Lines[selStartPt.Y] := sLead;
      for i := selEndPt.Y downto selStartPt.Y+1 do Lines.Delete(i);
      Lines[selStartPt.Y] := Lines[selStartPt.Y] + sTrail;
    end;
  finally
    Lines.EndUpdate;
  end;
  SetCaretPtInternal(selStartPt, true);
  UndoItem.NewSelStart := SelStart;
  UndoItem.NewSelEnd := SelStart; //really unused
  UndoItem.NewCaretPt := CaretPt;
  UndoList.Add(UndoItem);
end;
//------------------------------------------------------------------------------
...
begin
  ...
  UndoItem.UndoType := utSetSel;
  UndoItem.OldSelEnd := fSelEnd;
  UndoItem.OldCaretPt := fCaretPt;
  ...
  UndoItem.NewSelEnd := fSelEnd;
  UndoItem.AddedText := selText;
  fUndo.Add(UndoItem);
  ScrollCaretIntoView;
end;
//------------------------------------------------------------------------------
...
var
  ...
  UndoItem: TUndoItem;
begin
  if fReadOnly or not CharInSet(Key, [DEL, BACKSPC, TAB, RETURN, #32..#255]) then exit;
  if (fLines.Count = 0) then
  begin
    if CharInSet(Key, [DEL, BACKSPC]) then exit;
    //add a new line ...
    fLines.Add('');
    fCaretPt.X := 0;
  ...
  UndoItem.UndoType := utAddChar;   //will change if key is DEL or BACKSPC
  UndoItem.AddedText := Key;
  UndoItem.OldSelEnd := fSelEnd;   //updated if DeleteSelection called
  UndoItem.OldCaretPt := fCaretPt; //updated if DeleteSelection called
  ...
  case Key of
    DEL:
      ...
      begin
        DeleteSelection;
        UndoItem.UndoType := utDelChar;
      end
      //'delete' at end of line so join with following line ...
      else if fCaretPt.X = fLines.LineObj[fCaretPt.Y].fLineLen then
      begin
        if fCaretPt.Y = fLines.count -1 then exit;
        UndoItem.UndoType := utDelChar;
        UndoItem.DeletedText := #13;
        if fCaretPt.X = 0 then
          fLines.Delete(fCaretPt.Y) //delete empty line
        else if fLines.LineObj[fCaretPt.Y+1].fLineLen = 0 then
          fLines.Delete(fCaretPt.Y+1) //delete empty following line
        else
        begin
      ...
      begin
        ...
        UndoItem.UndoType := utDelChar;
        UndoItem.DeletedText := s[fCaretPt.X+1];
    ...
    BACKSPC:
      ...
      //'backspace' at start of line so join with preceeding line ...
      ...
      begin
        ...
        UndoItem.DeletedText := #13;
        UndoItem.OldCaretPt := fCaretPt;
        UndoItem.OldSelEnd := CharPosFromCharPt(fCaretPt);
        fUndo.Add(UndoItem); //behaves like a DEL while at end of previous line
      end else
      //just 'delete' a char in the line ...
      begin
        s := fLines[fCaretPt.Y];
        UndoItem.UndoType := utDelChar;
        UndoItem.DeletedText := s[fCaretPt.X];
        delete(s,fCaretPt.X,1);
        fLines[fCaretPt.Y] := s;
        dec(fCaretPt.X);
        UndoItem.OldSelEnd := fSelEnd-1; //treat it as if a DEL key
        dec(UndoItem.OldCaretPt.X);
        fUndo.Add(UndoItem);
      end;
    RETURN:
      begin
  ...
  ScrollCaretIntoView;
  DoOnCaretPtChange;
  if UndoItem.UndoType = utAddChar then
  begin
    UndoItem.NewSelEnd := fSelEnd;
    UndoItem.NewCaretPt := fCaretPt;
    fUndo.Add(UndoItem);
  end;
end;
//------------------------------------------------------------------------------
...
procedure TCodeEdit.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
  if (Operation = opRemove) and (AComponent = fPartner) then fPartner := nil;
end;
//------------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    //first paint the the line without any selection ...
    if assigned(fOnPaintLine) then
    begin
      handled := false;
      fOnPaintLine(self, LineNo, rec, lft, handled);
    end;

    //Now, if the line contains selected text, repaint the selection ...
    ...
    begin
      ...
      //and reuse relSelEnd to represent sellength ...
      ...
      rec.right := rec.Left + FCharWidth*relSelEnd;
