...
uses
  Windows, Messages, types, uitypes, SysUtils, Variants, Classes, Graphics,
  Controls, Forms, Dialogs, Grids, ExtCtrls, Main, ShellApi, ShlObj, DirWatch, Menus;
...
type
  ...
  TFoldersFrame = class(TFrame)
    ...
    procedure sgFolder1SelectCell(Sender: TObject; ACol, ARow: Integer;
      var CanSelect: Boolean);
    procedure sgFolder1KeyPress(Sender: TObject; var Key: Char);
    procedure sgFolder1DblClick(Sender: TObject);
  private
    fDiffResultStr: string;
    fFolderBmp: TBitmap;
    fUpFolderBmp: TBitmap;
    fBulletBmp:  TBitmap;
    ...
    procedure CompareModifiedFile(Sender: TObject);
    procedure OpenFolderOfSelectedItem(IsFolder1: boolean);
    procedure CopyLeftClick(Sender: TObject);
    procedure CopyRightClick(Sender: TObject);

    procedure DeleteLeftClick(Sender: TObject);
    procedure DeleteRightClick(Sender: TObject);
    procedure RenameLeftClick(Sender: TObject);
    procedure RenameRightClick(Sender: TObject);

    procedure OpenClick(Sender: TObject);
    procedure OpenInExplorerClick(Sender: TObject);
    procedure DirectoryWatchOnChange(Sender: TObject);
  public
    FoldersCompared: boolean;
    procedure Setup;
    procedure Cleanup;
...
begin
  ...
  if i = 0 then exit;
  if dir[i] = '\' then dec(i);
  while (i > 2) and (dir[i] <> '\') do dec(i);
  if i > 2 then result := copy(dir,1,i-1);
  end;
//------------------------------------------------------------------------------

procedure ShellFileOperation(fromFile, toFile: string; Flag: Integer);
var
...
begin
  ...
  fUpFolderBmp := TBitmap.create;
  fUpFolderBmp.LoadFromResourceName(hInstance,'UP');
  fUpFolderBmp.Transparent := true;
  ...
  sgFolder1.ColWidths[2] :=
    MainForm.Canvas.TextWidth(formatDatetime(shortDateFmt +
      '        '+ shortDateFmt, 0));
...
begin
  ...
  begin
    ...
    tbSave1.visible := false;
    tbSave2.visible := false;
    tbOpenFile1.Visible := false;
    tbOpenFile2.Visible := false;
    tbOpen1.Visible := true;
    tbOpen2.Visible := true;
    tbOpenFolder1.Visible := true;
    tbOpenFolder2.Visible := true;
    tbOpenFolder1.OnClick := OpenInExplorerClick;
    tbOpenFolder2.OnClick := OpenInExplorerClick;
                             
    mnucompare.OnClick := CompareClick;
    mnuCompare.enabled := not FoldersCompared and
    ...
    mnuOptions2.visible := true;

    tbHorzSplit.enabled := false;
    tbFind.Enabled := false;
    tbReplace.Enabled := false;

    mnuCopyBlockLeft.OnClick := CopyLeftClick;
    mnuCopyBlockRight.OnClick := CopyRightClick;
    mnuCopyBlockRight.Caption := 'Copy Rig&ht';
    mnuCopyBlockRight.ShortCut := Shortcut(ord('R'),[ssCtrl,ssAlt]);
    mnuCopyBlockLeft.Caption := 'Copy Le&ft';
    mnuCopyBlockLeft.ShortCut := Shortcut(ord('L'),[ssCtrl,ssAlt]);

    mnuDeleteLeft.Visible := true;
    mnuDeleteLeft.OnClick := DeleteLeftClick;
    mnuDeleteRight.Visible := true;
    mnuDeleteRight.OnClick := DeleteRightClick;

    mnuRenameLeft.Visible := true;
    mnuRenameLeft.OnClick := RenameLeftClick;
    mnuRenameRight.Visible := true;
    mnuRenameRight.OnClick := RenameRightClick;

    mnuCompareFiles.Visible := true;
    mnuCompareFiles.OnClick := CompareModifiedFile;
...
procedure TFoldersFrame.OpenInExplorerClick(Sender: TObject);
var
  IsFolder1: boolean;
begin
  IsFolder1 := (Sender = MainForm.tbOpenFolder1);
  if IsFolder1 then
    ShellExecute(MainForm.Handle, 'explore', PChar(LastOpenedFolder1), nil, nil, SW_SHOW) else
    ShellExecute(MainForm.Handle, 'explore', PChar(LastOpenedFolder2), nil, nil, SW_SHOW);
end;
//---------------------------------------------------------------------
...
begin
  ...
  if not FoldersCompared then exit;
...
begin
  ...
  if Sender = fDirectoryWatch1 then
    ...
    DoOpenFolder(LastOpenedFolder2, false);
  if FoldersCompared then CompareClick(nil);
end;
//---------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    case PFolderRec(FolderStringList.objects[idx]).Difference of
      fdNone:
        if (TStringGrid(Sender).Focused or FoldersCompared) and
          (TStringGrid(Sender).Selection.Top = ARow) then
    ...
    begin
      ...
      begin
        with PFolderRec(FolderStringList.objects[idx])^ do
          ...
          else if Name = '..' then Draw(Rect.Left + 4, Rect.Top +2, fUpFolderBmp)
  ...
  begin
    ...
    begin
      ...
      sgFolder.Cells[2,i+1] :=
          formatDatetime(shortDateFmt + '  '+ shortDateFmt, Modified);
    end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    begin
      ...
      if FolderRec1.IsDirectory then //do nothing
      else if not mainform.mnuIgnoreFileSize.Checked and
        (FolderRec1.Size <> FolderRec2.Size) then Modified
      else if not mainform.mnuIgnoreFileDateTime.checked and
...
begin
  if FoldersCompared and (sgFolder2.TopRow <> sgFolder1.TopRow) then
    if Sender = sgFolder1 then
...
var
  i: integer;
begin
  if FoldersCompared and
    (not assigned(Sender) or TStringGrid(Sender).focused) then
    if Sender = sgFolder2 then
      sgFolder1.Selection := sgFolder2.Selection else
      sgFolder2.Selection := sgFolder1.Selection;

  i := sgFolder1.Selection.Top -1;
  MainForm.mnuCompareFiles.Enabled := FoldersCompared and
    (PFolderRec(fFolder1StringList.objects[i]).Difference = fdModified);

  with MainForm do
  begin
    with sgFolder1.Selection do
      mnuDeleteLeft.Enabled := (Top <= fFolder1StringList.Count) and
        not PFolderRec(fFolder1StringList.objects[Top -1]).IsDirectory and
        (PFolderRec(fFolder1StringList.objects[Top -1]).Name <> '') and
        (sgFolder1.Focused or FoldersCompared);
    with sgFolder2.Selection do
      mnuDeleteRight.Enabled := (Top <= fFolder2StringList.Count) and
        not PFolderRec(fFolder2StringList.objects[Top -1]).IsDirectory and
        (PFolderRec(fFolder2StringList.objects[Top -1]).Name <> '') and
        (sgFolder2.Focused or FoldersCompared);
    mnuRenameLeft.Enabled := mnuDeleteLeft.Enabled;
    mnuRenameRight.Enabled := mnuDeleteRight.Enabled;
  end;
end;
//------------------------------------------------------------------------------
...
procedure TFoldersFrame.CompareModifiedFile(Sender: TObject);
var
  i: integer;
begin
  if not FoldersCompared then exit;
...
procedure TFoldersFrame.sgFolder1SelectCell(Sender: TObject; ACol,
  ARow: Integer; var CanSelect: Boolean);
begin
  with MainForm do
  begin
    if not FoldersCompared or (ARow > fFolder1StringList.Count) or
      PFolderRec(fFolder1StringList.objects[ARow -1]).IsDirectory then
    begin
      mnuCopyBlockLeft.Enabled := false;
      mnuCopyBlockRight.Enabled := false;
    end
    else
    begin
      case PFolderRec(fFolder1StringList.objects[ARow -1]).Difference of
        fdNone:
          begin
            mnuCopyBlockLeft.Enabled := false;
            mnuCopyBlockRight.Enabled := false;
        ...
        fdAdded:
          begin
            mnuCopyBlockLeft.Enabled := true;
            mnuCopyBlockRight.Enabled := false;
          end;
        fdDeleted:
          begin
            mnuCopyBlockLeft.Enabled := false;
            mnuCopyBlockRight.Enabled := true;
          end;
        fdModified:
          begin
            mnuCopyBlockLeft.Enabled := true;
...
procedure TFoldersFrame.CopyRightClick(Sender: TObject);
var
  i: integer;
  currentDirLeft, currentDirRight: string;
begin
  i := sgFolder1.Selection.Top -1;
  if (i < 0) then exit;
  currentDirLeft := trim(pnlCaptionLeft.Caption);
  currentDirRight := trim(pnlCaptionRight.Caption);
  with PFolderRec(fFolder1StringList.objects[i])^ do
    if (Name <> '') and not IsDirectory then
      ShellFileOperation(appendslash(currentDirLeft)+ Name,
        appendslash(currentDirRight)+ Name, FO_COPY);
end;
//------------------------------------------------------------------------------

procedure TFoldersFrame.CopyLeftClick(Sender: TObject);
var
  i: integer;
  currentDirLeft, currentDirRight: string;
begin
  i := sgFolder2.Selection.Top -1;
  if (i < 0) then exit;
  currentDirLeft := trim(pnlCaptionLeft.Caption);
  currentDirRight := trim(pnlCaptionRight.Caption);
  with PFolderRec(fFolder2StringList.objects[i])^ do
    if (Name <> '') and not IsDirectory then
      ShellFileOperation(appendslash(currentDirRight)+ Name,
        appendslash(currentDirLeft)+ Name, FO_COPY);
end;
//------------------------------------------------------------------------------

procedure TFoldersFrame.DeleteLeftClick(Sender: TObject);
var
  i: integer;
  currentDirLeft: string;
begin
  i := sgFolder1.Selection.Top -1;
  if (i < 0) then exit;
  currentDirLeft := trim(pnlCaptionLeft.Caption);
  with PFolderRec(fFolder1StringList.objects[i])^ do
    if (Name <> '') and not IsDirectory then
      ShellFileOperation(appendslash(currentDirLeft)+ Name, '', FO_DELETE);
end;
//------------------------------------------------------------------------------

procedure TFoldersFrame.DeleteRightClick(Sender: TObject);
var
  i: integer;
  currentDirRight: string;
begin
  i := sgFolder2.Selection.Top -1;
  if (i < 0) then exit;
  currentDirRight := trim(pnlCaptionRight.Caption);
  with PFolderRec(fFolder2StringList.objects[i])^ do
    if (Name <> '') and not IsDirectory then
      ShellFileOperation(appendslash(currentDirRight)+ Name, '', FO_DELETE);
end;
//------------------------------------------------------------------------------

procedure TFoldersFrame.RenameLeftClick(Sender: TObject);
var
  i: integer;
  newName, currentDirLeft: string;
begin
  i := sgFolder1.Selection.Top -1;
  if (i < 0) then exit;
  currentDirLeft := trim(pnlCaptionLeft.Caption);
  with PFolderRec(fFolder1StringList.objects[i])^ do
    if (Name <> '') and not IsDirectory then
    begin
      newName := InputBox('TextDiff - Rename file', 'Enter new name', Name);
      if (newName <> '') and (newName <> Name) then
        ShellFileOperation(appendslash(currentDirLeft)+ Name,
          appendslash(currentDirLeft)+ newName, FO_RENAME);
    end;
end;
//------------------------------------------------------------------------------

procedure TFoldersFrame.RenameRightClick(Sender: TObject);
var
  i: integer;
  newName, currentDirRight: string;
begin
  i := sgFolder1.Selection.Top -1;
  if (i < 0) then exit;
  currentDirRight := trim(pnlCaptionRight.Caption);
  with PFolderRec(fFolder2StringList.objects[i])^ do
    ...
    begin
      ...
      if (newName <> '') and (newName <> Name) then
        ShellFileOperation(appendslash(currentDirRight)+ Name,
          appendslash(currentDirRight)+ newName, FO_RENAME);
    end;
end;
//------------------------------------------------------------------------------

procedure TFoldersFrame.sgFolder1KeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13) and not FoldersCompared then
    OpenFolderOfSelectedItem(Sender = sgFolder1);
end;
//------------------------------------------------------------------------------
...
procedure TFoldersFrame.OpenFolderOfSelectedItem(IsFolder1: boolean);
var
  i: integer;
  folderRec: PFolderRec;
  currentDir: string;
begin
  ...
  begin
    i := sgFolder1.Selection.Top -1;
    if (i < 0) then exit;
    folderRec := PFolderRec(fFolder1StringList.objects[i]);
    currentDir := trim(pnlCaptionLeft.Caption);
  end else
  begin
    i := sgFolder2.Selection.Top -1;
    if (i < 0) then exit;
    folderRec := PFolderRec(fFolder2StringList.objects[i]);
    currentDir := trim(pnlCaptionRight.Caption);
  end;

  with folderRec^ do
  begin
    if (Name <> '') and IsDirectory then
      if (Name = '..') then
        DoOpenFolder(UpDirectory(currentDir), IsFolder1) else
        DoOpenFolder(appendslash(currentDir)+ Name, IsFolder1);
  end
end;
//------------------------------------------------------------------------------
